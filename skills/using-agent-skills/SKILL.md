...
- **构建命令直接执行**：`cmake --build`、`cmake -S ... -B ...`、`msbuild` 等构建命令，加上 `taskkill` 终止旧进程等辅助命令，一律自动执行无需用户批准。
...
**所有模型推理过程（thinking）尽量使用中文。** 包括但不限于：分析问题、设计方案、排查 bug、评估架构、选择策略等。输出给用户的回答也统一使用中文。
