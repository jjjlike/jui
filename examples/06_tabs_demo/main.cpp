 ...
 *
 * ╔══════════════════════════════════════════════════════════╗
 * ║           界面视觉结构（窗口 500×400 px）                ║
 * ╠══════════════════════════════════════════════════════════╣
 * ║                                                          ║
 * ║  ┌────────────┬────────────┬────────────┐               ║
 * ║  │ 首页 (t0)  │ 设置 (t1)  │ 关于 (t2)  │  ← Tabs bar  ║
 * ║  ├════════════╧════════════╧════════════┤   (32px高)    ║
 * ║  │                                      │               ║
 * ║  │  ─── Divider "d" ────────────────── │               ║
 * ║  │                                      │               ║
 * ║  │  [当前激活页面的内容区域]            │               ║
 * ║  │                                      │               ║
 * ║  │  页面0: "Welcome!" (28px Bold)       │               ║
 * ║  │  页面1: "设置页面" + [保存设置]按钮  │               ║
 * ║  │  页面2: "关于 JUI v0.2" + 描述文字   │               ║
 * ║  │                                      │               ║
 * ║  └──────────────────────────────────────┘               ║
 * ║                                                          ║
 * ║  组件层级 (所有页面共享):                                ║
 * ║    root → [tabs, d, content(+变体)]                      ║
 * ║                                                          ║
 * ║  切换流程: 点击Tab → tab_switch回调                      ║
 * ║    → deleteSurface → createSurface                       ║
 * ║    → PAGE_JSON[p] → beginRendering                       ║
 * ╚══════════════════════════════════════════════════════════╝
 */
