 ...
 * ║  Column "root"                                       ║
 * ║  ┌──────────────────────────────────────────────┐    ║
 * ║  │  Text "title" (path绑定: /user/name)         │    ║
 * ║  │  ┌──────────────────────────────────────┐   │    ║
 * ║  │  │  "Guest" ← 当前数据模型中的值        │   │    ║
 * ║  │  └──────────────────────────────────────┘   │    ║
 * ║  │     ↑ 绑定到 DataModel["/user/name"] ↑       │    ║
 * ║  │                                              │    ║
 * ║  │  ┌──────────┬──────────┬──────────────┐     │    ║
 * ║  │  │ btn1     │ btn2     │ btn3          │     │    ║
 * ║  │  │ "Alice"  │ "Bob"    │ "Charlie"     │     │    ║
 * ║  │  │action=alice│action=bob│action=charlie│     │    ║
 * ║  │  └──────────┴──────────┴──────────────┘     │    ║
 * ║  │     ↓点击    ↓点击      ↓点击                │    ║
 * ║  │   setValue(/user/name, 对应名字)              │    ║
 * ║  │   → title 自动重新渲染显示新名字              │    ║
 * ║  └──────────────────────────────────────────────┘    ║
 * ║                                                      ║
 * ║  组件层级: root → [title(path), btn1, btn2, btn3]    ║
 * ║  数据流: 按钮点击 → setValue → title自动更新         ║
 * ╚══════════════════════════════════════════════════════╝
 */
